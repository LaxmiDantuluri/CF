...
#### TABLES USED ################
#v_enginefam = "GENX"
v_phm = v_sourceHiveDataSchemaName1+'.phm_flight_cf_cdc_stg_genx'
...
################# checking to see partitions ####################################
    #lsStrPartitions  = COMMON_FLIGHT.select("best_yearmonth").distinct().rdd.flatMap(lambda x: x).collect()
    #print 'partitions'
    #print lsStrPartitions 
...
#`save COMMON FLIGHT as external file
    COMMON_FLIGHT.write.mode("overwrite").partitionBy("best_yearmonth").parquet( v_finalCFTablefullpath )
